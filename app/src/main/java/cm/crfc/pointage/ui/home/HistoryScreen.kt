...
fun HistoryScreen(
    ...
    ScreenList {
        item {
            HeaderCard(
                title = "Historique",
                subtitle = "${filtered.size} rapport(s) affiches",
                actions = {
                    OutlinedButton(
                        onClick = {
                            if (filtered.isEmpty()) {
                ...
                CrfcCard(
                    ...
                    Row(
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            Text(formatDisplayDate(report.date), style = androidx.compose.material3.MaterialTheme.typography.titleMedium)
                            StatusBadge(
                        ...
                        if (user.role == UserRole.ADMIN) {
                            IconButton(onClick = {
                                ...
                                Icon(Icons.Rounded.Delete, contentDescription = null, tint = androidx.compose.material3.MaterialTheme.colorScheme.error)
                            }
                        }
                    }
                    ...
                    Row(
                        ...
                        StatPill(Icons.Rounded.Schedule, "${report.lateEntries.size} retard(s)", extra.late)
                        StatPill(Icons.Rounded.WarningAmber, "${report.absenceEntries.size} absence(s)", extra.absent)
                        StatPill(Icons.Rounded.Group, "${report.visitorCount} visiteur(s)", androidx.compose.material3.MaterialTheme.colorScheme.primary)
                    }
                }
            }
        }
